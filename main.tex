...
\lhead{Tesi di Laurea Magistrale}
\cfoot{\thepage}
...
\end{center}
Even if Earth isn't new to climate changes as said before, evidences like ice cores drawn from Greenland or Antarctica, tree rings,inco ocean sediments, reveal that the current warming is occurring almost ten times faster than the ones happened after ice ages, as seen in Figure \ref{climate_change_nasa}.
\newline
\begin{figure}[ht]
...
\newline
That is because the data on which we are trying to apply deep learning techniques are about weather, therefore in order to have statistically significant data we must at miminum have a whole year worth of measurements, so that every season would be monitored equally.
\newline
\newline
Due to the unavailability of good data, I started to search for similar projects and luckily I found OpenSense project from Zurich \cite{opensense}. It is a research group that aims at investigating community-based sensing using a network of wireless sensors to monitor air pollution.
\newline
...
Being OpenSense a professional organization, they have been able to develop a far more efficient and reliable monitoring station than the proof-of-concept me and my colleagues built for the CINI Challenge 2018. Their stations are equipped with very good sensors and are also really well engineered in order to avoid water and dust entering the insides, see Figure \ref{opensense_station}.
...
\newline
\newline
They propose an approach based on mobile monitoring stations in order to enhance the spatial resolution by trading off temporal resolution. The basic idea is the same as CleanAir Project: \textit{a fleet of mobile monitoring stations can cover a large area without the need of a lot of elements}.
...
The measured UFP concentrations are projected on a grid with 100m x 100m cells that completely covers Zurich. In their deployement they had a large number of measurement location but they found out that with many locations it was difficult to get predictions with high $R^2$ values; they later found empirically that restricting to use the first 200 cells with higher number of measurements brought them the best results.
...
\begin{itemize}
    ...
    \begin{align}
        \notag \Bar{y} = \frac{1}{n}\sum_{i=1}^n c_i^m \\
        R^2 = \frac{\sum_{i=1}^n (c_i^m - c_i^P)^2}{\sum_{i=1}^n (c_i^m - \Bar{y})^2}
    \end{align}
\end{itemize}
...
\newline
\newline
The biggest challenge in data science is always about the data: having large amounts of good quality data is the \textit{conditio sine qua non} to solve a problem. In my case to have enough data was simply impossible, due to the lack of reliable monitoring stations (me and my colleagues built one prototype only) and also at least a year of measurements is mandatory to let the system learn how seasons progress and relate to each other.
...
\begin{itemize}
    ...
    \item \textbf{time\_index}. An integer representing the time index for a sample, i.e. in the daily time frame all mondays have a time\_index of 1, tuesdays have 2 and so on, in order to help the model catch correlations between similar time points.
\end{itemize}
...
\begin{itemize}
    ...
    \item seasonal concentrations: 4 rows, average concentrations for each season, the northern meteorological division \cite{meteorological-seasons} is used to assign each measurement to a season (all seasons are exactly 3 months long, for example spring lasts from 01/03 to 31/05)
\end{itemize}
...
\end{itemize}
\subsection{UFP concentration analysis}
\begin{figure}[ht]
    \centering
    \includesvg[width=0.95\textwidth]{imgs/ch_4/average_UFP_all_city.svg}
...
\begin{figure}[ht]
    \centering
    \includesvg[width=0.95\textwidth]{imgs/ch_4/std_ufp_all_city.svg}
    \caption{The standard deviation of UFP concentrations for each time frame.} 
    \label{cleanair_std_ufp_city}
\end{figure}
Figure \ref{cleanair_std_ufp_city} shows the standard deviation of UFP concentrations in the whole city for each time frame, in light of these plots some considerations can be made over each time frame:
\begin{itemize}
    \item \textbf{Seasonal}: As expected summer is the season with less pollution thanks to the lack of house heating, this hypothesis is confirmed by the increasing trend as the seasons get colder.
    \item \textbf{Monthly}: The final month of each season sees a variance increase due to the changing temperatures and the consequent change in people's behaviour, which happens asynchronously around the city and therefore pollution is distributed in a much sparser way than the other months.
    \item \textbf{Weekly}: This time frame is the more noisy one, as for its variance. The same "spiking" trend as for the monthly time frame can be noticed in a greater time resolution, last weeks of the final month of each season present a higher variance; the mean value also confirms the seasonal trend.
    \item \textbf{Daily}: During the average work days pollution levels have a relatively stable trend while weekends see a good decrease of concentrations, most likely due to people not having to use carbon-fueled transports to reach their workplace.
\end{itemize}
\begin{figure}[ht]
    \centering
    \includegraphics[width=\textwidth]{imgs/ch_4/ufp_average_all_city.png}
    \caption{The yearly average UFP concentration for each cell.} 
    \label{average_ufp_heatmap}
\end{figure}
Figure \ref{average_ufp_heatmap} shows the geographical distribution of yearly UFP concentrations across the whole city of Zurich, to further asses the distribution of such particles.
\newpage
...
The framework used to build, train and evaluate the Convolutional Neural Network described in \ref{cleanair:cnn_schema} is PyTorch. The motivations behind the choice of this framework instead of something more popular such as Google's Tensorflow is dictated by the customization needs of this project.
\newline 
...
\subsubsection{Map Images}
Each cell has an attribute that references to its corresponding map image, saved in the file system of the server running MongoDB and also present in the 4 aggregated files; those images have been gathered by means of one of the many OpenStreetMap Map Tile Servers \cite{openstreetmap-tile-servers}.
\newline
\newline
OpenStreetMap provides map tiles to its users by means of the Tile Map Server protocol \cite{openstreetmap-tmp-protocol} also known as TMP, which is a protocol for serving maps as tiles i.e. splitting the map up into a pyramid of images at multiple zoom levels. 
\newline
\newline
The map tiles are usually images with a resolution of 256x256 pixels and are fetched via \textit{tile numbers}, which is a referencing system for map tiles at different levels of zooms. The Tile Server used to fetch the map images for this project has been the following:
\begin{verbatim}
    https://tiles.wmflabs.org/osm-no-labels/${z}/${x}/${y}.png
\end{verbatim}
where \textit{z} is the zoom level and \textit{x, y} are the coordinates in tile numbers. The conversion from latlon format to tile numbers is performed in according to the SlippyMap standard as explained in \cite{openstreetmap-latlon-to-tms}:
\begin{align}
    x = \Bigg[\frac{lon + 180}{360} \cdot 2^{zoom}\Bigg] \\
    y = \Bigg[\left(1 - \frac{\ln(\tan(lat \cdot \frac{\pi}{180}))}{\pi} + \frac{1}{\cos(lat \cdot \frac{\pi}{180})}\right) \cdot 2^{z-1}\Bigg]
\end{align}
To help the model better understand the environment surrounding a cell, the images are fetched using a zoom level of 15, which means they cover more or less an area of 300m x 300m; this increase in size helped the model to get closer to the basic idea of this project: \textit{using small portions of map, learn how the pollutants move in topologically similar zones}.  

\subsubsection{Item fetching}
When the method \texttt{\_\_getitem(idx)\_\_} is invoked the Dataset class has to return the sample corresponding to the specified index \textit{idx}; in the context of CleanAir the fetching of a sample consisted in retrieving the idx-th line from the DataFrame, reading the corresponding map image (via SciKitLearn's imread \cite{skimage-imread} function) and returning a dictionary such as the following:
\begin{verbatim}
    sample = {
        image: torch.Tensor,
...
\end{verbatim}
Where \texttt{image} is a Tensor representing each channel as a matrix of pixel values, \texttt{weather\_data} is a flattened Tensor containing the weather measurements (explained in \ref{cleanair:dataset}) for that cell at a given time point, and \texttt{ufp\_label} is the average UFP concentration to predict.

\subsection{Training}
The training process has been feasible thanks to the help of the GARR Cloud \cite{garr-cloud}, who provides a Kubernetes environment to access cloud computing instances and furthermore powerful GPUs, fundamental to train deep neural networks much faster. 
...
To train a model means using some sort of error measure and tweak the model's parameters accordingly. In the context of Regression the functions available are all about measuring the difference between the predicted and original values, the difference lies in the implications of using one function or another.
\newline
\newline
...
\newline
\newline
In \ref{cleanair:stat-analysis} the mean value and variance of UFP values to predict has been analyzed, and due to the relatively high variance of this data set (which was expected, because the objects of the study are particles moving around the air) the L1 loss function has been chosen to train the network:
\begin{align}
    \text{MAE} = \sum_{i=o}^n | y_i - h(x_i)|
\end{align}

\subsubsection{Adam Optimizer}
The optimizer used in this training process has been one of the most used and known ones, Adam \cite{adam}. It is an algorithm built for first-order gradient-based optimization of stochastic objective functions, based on adaptive estimates of lower-order moments. The method is straightforward to implement, is computationally efficient, has little memory requirements, is invariant to diagonal rescaling of the gradients, and is well suited for problems that are large in terms of data and/or parameters.
\newline
\newline
Adam is different to classical stochastic gradient descent because it has an \textbf{adaptive learning rate}, hence it changes the learning rate value during training based on both the average first moment and the second moment of the gradients.

The authors describe it as combining the features of two other extensions of the stochastic gradient descent:
\begin{itemize}
    \item \textbf{Adaptive Gradient Algorithm} (AdaGrad): maintains a per-parameter learning rate that improves performance on problems with sparse gradients.
    \item \textbf{Root Mean Square Propagation} (RMSProp): per-parameter learning rate adapted based on the average of recent magnitudes of the gradients for the weight (essentially how quickly it is changing). This approach does well on online and non-stationary problems, for example on noisy data sets.
\end{itemize}
The algorithm consists in calculating an exponential moving average of the gradient and the squared gradient with the parameters $\beta_1$ and $\beta_2$ in charge of handling the decay rates of these moving averages. Those parameters are recommended to start close to 1.0 in order to get a bias of the moment estimates towards zero, the bias is overcome by first calculating the biased estimates before recalculating the bias-corrected estimates.

\subsection{Evaluation}
The model's performances have been evaluated with the same metrics used by OpenSense, in order to being able to confront the results: \textit{Root Mean Square Error (RMSE)}, \textit{Coefficient of determination $R^2$} and the \textit{Factor of 2 measure (FAC2)}.
\subsubsection{Root Mean Squared Error}
It quantifies the difference between the predicted and real UFP value:
\begin{align}
    \text{RMSE} = \sqrt{\frac{\sum_{i=1}^N (c_i^P - c_i^m)^2}{N}}
\end{align}
...
\subsubsection{Adjusted coefficient of determination}
...
\begin{align}
    ...
    R^2 = \frac{\sum_{i=1}^n (c_i^m - c_i^P)^2}{\sum_{i=1}^n (c_i^m - \Bar{y})^2}
\end{align}
\subsubsection{Factor of 2 measure}
Quantitatively analyzes scatter plots of predictions by evaluating the fraction of data points that lie inside the \textit{factor two} area, i.e. the fraction of data that satisfies the following:
\begin{align}
    0.5 \le \frac{c_i^P}{c_i^m} \le 2.0
\end{align}
...
\newline
\newline
Even though the two projects are similar, some fundamental \textbf{differences} lie in how the data are aggregated both spatially and temporally. The \textbf{temporal aggregation} performed by OpenSense produced one model per time unit for each time agglomeration considered (meaning that they produced one model per day for the daily time agglomeration, and so on) whereas CleanAir produces one model for each time agglomeration, capable of handling the average time units inside of it (the daily time frame produces one model capable of handling each average day of the week).

The differences about \textbf{spatial aggregation} are even greater: OpenSense states \cite{Deriving-High-Res-Maps} that they found that using only the first 200 cells with the highest number of measurements to train and test their model gives the best results, CleanAir on the other hand uses almost 20000 cells in the 100m x 100m resolution obtaining comparable results even if using a smaller number of measurements per cell than OpenSense.
\newline
\newline
To test the model three different grid resolutions have been experimented: 100x100m, 250x250m, 500x500m.
\section{Results discussion}
Tables \ref{cleanair:results-table-100m}, \ref{cleanair:results-table-250m}, \ref{cleanair:results-table-500m} show the results of each model at the three different spatial resolutions experimented: 100m x 100m, 250m x 250m, 500m x 500m. A lot more resolution and even cell shapes could be experimented, amongst the ones considered in this research the 250m x 250m cells have proven to be the most accurate, therefore this could be a good starting point for further fine-tuning processes.
\begin{table}[ht]
    \centering
    \begin{tabular}{@{}l|llll@{}}
    \toprule
         & Daily & Weekly & Monthly & Seasonal \\ \midrule
    FAC2 & 0.987     & 0.934      & 0.973       & 0.992        \\
    RMSE & 2932.414     & 4815.764      & 3354.047       & 2316.726        \\
    R2   & 0.372     & 0.287      & 0.532       & 0.526        \\ \bottomrule
    \end{tabular}
    \caption{Metrics values for each model, 100m x 100m resolution}
    \label{cleanair:results-table-100m}
\end{table}
\begin{table}[ht]
    \centering
    \begin{tabular}{@{}l|llll@{}}
    ...
    R2   & 0.586     & 0.516      & 0.597       & 0.815        \\ \bottomrule
    \end{tabular}
    \caption{Metrics values for each model, 250m x 250m resolution}
    \label{cleanair:results-table-250m}
\end{table}
\begin{table}[ht]
    \centering
    \begin{tabular}{@{}l|llll@{}}
    ...
    R2   & 0.292     & 0.317      & 0.327       & 0.542        \\ \bottomrule
    \end{tabular}
    \caption{Metrics values for each model, 500m x 500m resolution}
    \label{cleanair:results-table-500m}
\end{table}
Figure \ref{opensense_results} shows the metrics values obtained by OpenSense, for each time aggregation level they considered; the sub-weekly models show weaknesses mostly in terms of $R^2$.
\begin{figure}[ht]
    \centering
    \includegraphics[width=\textwidth]{imgs/ch_5/OpenSense_results.jpg}
    \caption{Results obtained by OpenSense reasearch team.} 
    \label{opensense_results}
\end{figure}

According to \cite{air-quality-evaluation} one of the most important factor when evaluating an air quality model is the FAC2 score; considering that my system has FAC2 scores slightly higher (in average) than the ones obtained by OpenSense, and given that OpenSense claims to have reached \textit{close to state-of-the-art results}, it is safe to state that CleanAir Project's approach is a valid alternative to more traditional models and could be worth further studies.

\section{Advantages of using a Neural Networks approach}
Another fundamental difference between OpenSense and CleanAir are the \textbf{explanatory variables} \ref{tab:explanatory_vars}.
\newline
\newline
...
\newline
\newline
CleanAir adopts a completely automatized approach thanks to the use of a Convolutional Neural Network: \textit{the convolutional layers automatically extract a set of explanatory variable for each unique map portion}.
\newline
This approach has proven to be equally capable than the other one, but has the game changing advantage of \textbf{automatic explanatory variables extraction}, a quality that enables \textbf{CleanAir to be applied virtually anywhere in the world} where a fleet of mobile monitoring stations is available, \textbf{without the need to carefully craft and select explanatory variables manually}.
...
\begin{itemize}
    \item \textbf{Deploying physical monitoring stations}. Having an own fleet of monitoring station would mean being able to control the flow of data directly from the physical station, enabling experiments with different kinds of raw data pre-processing.
    \item \textbf{Experimenting with different CNN architectures}. For the purpose of this research a simple and well known architecture has been used for the Convolutional Neural Network such as the LeNet has been used, but in these last years tens of new and promising architectures have been proposed and could be worth of investigation.
    \item \textbf{Real-time map images}. A game changing data would be the ability of using real-time satellite images of the cells, in order to be able to associate to each measurement the image shoot by a satellite at the same hour. This would bring a lot of improvements to the model, because the feature extraction would produce a lot more informations (for example, the yellowing of the leaves on trees would induce the model to understand the passing of seasons, and therefore to catch possible relationships hidden before). 
\end{itemize}
...
\quote{\textit{Grazie a mio padre Aldo che mi insegna ad essere Uomo, grazie a mia madre Silvana che mi insegna ad Amare e grazie a mia sorella Elisa, che mi insegna a Ridere.}}

\begin{center}
    \thispagestyle{empty}
    \vspace*{\fill}
    \textit{La Via prosegue senza fine \\
